<div class="container mx-auto">
    <div class="grid grid-cols-4 gap-20">
        <div class="shadow-2xl h-min sticky top-0 border">
            @if ($div->avatar != NULL)
                @if (Storage::disk('public')->exists($div->avatar))
                <div class="p-5">
                    <img class="w-full h-36 mx-auto object-cover" src="{{asset('storage/'.$div->avatar)}}">
                </div>

                @endif
            @else
                <div class="p-5">
                    <img class="w-full h-auto object-cover" src="{{asset('images/'.$div->slug.'-division.jpg')}}">
                </div>

            @endif
            <div class="text-center px-5">
                <h1 class="text-red my-5 font-bold text-2xl"><a href="{{route('division.page',[$div->slug])}}">{{$div->name}} বিভাগ</a></h1>
                <h2 class="text-lg text-left">{{$div->about}}</h2>
            </div>
            <hr class="my-5">
            <div class="px-5 py-2">
            ...
            <div class="bg-green text-xl text-white flex justify-between items-center px-5 py-1">
                <span><a href="{{$div->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
                <span><a href="{{$div->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
                <span><a href="{{$div->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
                <span><a href="{{$div->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
            </div>
