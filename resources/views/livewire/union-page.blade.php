<div class="container mx-auto">
    <div class="grid grid-cols-4 gap-20">
        <div class="shadow-2xl h-min sticky top-0 overflow-y-scroll border pt-5">
            @if ($union->avatar != NULL)
                @if (Storage::disk('public')->exists($union->avatar))
                <img class="w-36 h-36 rounded-full border mx-auto object-cover border-red p-1" src="{{asset('images/'.$union->avatar)}}">
                @endif
            @else
                <img class="w-36 h-36 rounded-full border mx-auto object-cover border-red p-1" src="{{asset('images/'.$union->slug.'-upazila.jpg')}}">
            @endif
            <div class="text-center px-5">
                <h1 class="text-green font-bold text-2xl"><a href="{{route('union.page',[$div->slug, $dis->slug, $subdis->slug, $union->slug])}}">{{$union->}} উপজেলা</a></h1>
                <h2 class="text-lg text-left">{{$union->about}}</h2>
            </div>
            <hr class="my-5">
            <div class="px-5 py-2">
            ...
            <div class="bg-green text-xl text-white flex justify-between items-center px-5 py-1">
                <span><a href="{{$union->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
                <span><a href="{{$union->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
                <span><a href="{{$union->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
                <span><a href="{{$union->facebook}}"><i class="fa-brands fa-facebook"></i></a></span>
            </div>
